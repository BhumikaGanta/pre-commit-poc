...
export class TestController {
  private readonly _logger: ILogger<TestController> =
    new Logger<TestController>();

  constructor(
    @inject(TOKENS.TestServices) private readonly _testServices: ITestServices,
  ) {}

  getAll = (_req: Request, res: Response) =>
    respond(
      res,
      ResultAsync.fromSafePromise(this._testServices.GetAll()).andThen(
        (rows) => {
          if (rows.length === 0) {
  ...
  getById = (req: Request, res: Response) =>
    respond(
      ...
      parseId(req.params.id).asyncAndThen((id) =>
        ResultAsync.fromSafePromise(this._testServices.GetById(id)).andThen(
          (row) =>
  ...
  create = async (req: Request, res: Response) => {
    ...
    return respond(
      ...
      ResultAsync.fromSafePromise(
        this._testServices.Create(new Test(req.body)),
      ),
  ...
  update = async (req: Request, res: Response) => {
    ...
    return respond(
      ...
      parseId(req.params.id).asyncAndThen((id) =>
        ResultAsync.fromSafePromise(
          this._testServices.Update(id, new Test(req.body)),
        ).andThen((row) =>
          row ? okAsync(row) : errAsync(new NotFoundError("No data found")),
        ),
      ),
